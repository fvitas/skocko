...
export function SymbolIcon({ symbol, className }: SymbolIconProps) {
  // gradient ids must be unique per instance — duplicate ids resolve to the first
  // one in the document, and if that instance is display:none the fill vanishes
  const id = useId()
  if (symbol === 'skocko') {
    return (
      <svg viewBox="58 20 384 350" className={className} aria-hidden="true">
        <defs>
          <radialGradient id={`${id}-head`} cx="40%" cy="35%" r="65%">
            <stop offset="0%" stopColor="#ffde17" />
            <stop offset="85%" stopColor="#f2a900" />
            <stop offset="100%" stopColor="#d98200" />
          </radialGradient>
          <radialGradient id={`${id}-nose`} cx="35%" cy="35%" r="65%">
        ...
        <ellipse cx="250" cy="225" rx="28" ry="20" fill={`url(#${id}-nose)`} />
